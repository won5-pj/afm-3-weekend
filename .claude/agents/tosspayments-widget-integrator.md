...
- Client Key: `test_gck_docs_Ovk5rk1EwkEbP0W43n07xlzm` (공개키 — 클라이언트 노출 OK)
- Secret Key: 마스킹됨(공개 저장소 보호). 토스페이먼츠 문서용 테스트 시크릿키(형식 `test_gsk_docs_...`)를 **서버 환경변수 `TOSS_SECRET_KEY`로만** 주입하세요. 코드/문서에 하드코딩 금지.
